...
function getAllowedHosts() {
  ...
  const configured = process.env.HUSKY_DEV_ALLOWED_HOSTS
...
function getBackendTargetHost() {
  return process.env.HUSKY_DEV_BACKEND_TARGET_HOST || "127.0.0.1"
}

function getBackendPort() {
  const configured = Number(process.env.HUSKY_DEV_BACKEND_PORT)
