...
function DiffFileCard({
  ...
  const projectFilePreviewUrl = canReadProjectFile && projectId ? buildProjectRenderablePreviewUrl(projectId, file.path) : ""
  ...
  function handleOpenProjectFilePreview() {
    if (!projectFilePreviewUrl) return
    fileActions.onOpenPreview(file.path)
  }
...
function GitPanelImpl({
  ...
  const fileActions: DiffFileActions = useMemo(() => ({
    ...
    onCopyFilePath,
    onCopyRelativePath,
  }), [onOpenFile, onOpenPreview, onOpenInFinder, onDiscardFile, onIgnoreFile, onIgnoreFolder, onCopyFilePath, onCopyRelativePath])
