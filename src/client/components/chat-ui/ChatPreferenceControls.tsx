...
export function ChatPreferenceControls({
  ...
  const codexFastModeSupported = supportsCodexFastMode(model)
    ...
    <div className={cn("flex md:justify-center items-center gap-0.5", className)}>
      ...
      >
        {(close) => (
          selectedProvider === "claude"
            ...
            : CODEX_REASONING_OPTIONS
              .filter((effort) => codexReasoningOptions.includes(effort.id))
              .map((effort) => (
              <PopoverMenuItem
                key={effort.id}
                onClick={() => {
      ...
      {selectedProvider === "codex" ? (
        <InputPopover
          ...
          trigger={(
            <>
              {codexFastModeSupported && codexModelOptions?.fastMode
                ? <Gauge className="h-3.5 w-3.5" />
                : <Gauge className="h-3.5 w-3.5 -scale-x-100" />}
              <span>{codexFastModeSupported && codexModelOptions?.fastMode ? "Fast Mode" : "Standard"}</span>
            </>
          )}
          triggerClassName={codexFastModeSupported && codexModelOptions?.fastMode ? "text-emerald-500 dark:text-emerald-400" : undefined}
        >
          {(close) => (
            <>
              <PopoverMenuItem
