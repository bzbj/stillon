...
export function classifyAttachmentPreview(attachment: ChatAttachment): AttachmentPreviewTarget {
  ...
  if (isMarkdownAttachment(mimeType, extension)) {
...
export function classifyAttachmentIcon(attachment: ChatAttachment): AttachmentIconKind {
  ...
  if (isMarkdownAttachment(mimeType, extension)) return "markdown"
...
function isMarkdownAttachment(mimeType: string, extension: string) {
  return mimeType === "text/markdown" || MARKDOWN_EXTENSIONS.has(extension)
}
