...
export function buildWindowsServicePowerShell(
  ...
  return [
    ...
    "while ($true) {",
    "    $startedAt = Get-Date",
    "    $watchdogErrorActionPreference = $ErrorActionPreference",
    "    $serviceExitCode = $null",
    "    try {",
    "        # Windows PowerShell 5.1 promotes native stderr output to error records.",
    "        # StillOn uses stderr for recoverable application errors, so keep the",
    "        # watchdog strict everywhere except while waiting for the native process.",
    ...
    "    } finally {",
    "        $ErrorActionPreference = $watchdogErrorActionPreference",
    "    }",
    "    if ($null -eq $serviceExitCode) { $serviceExitCode = -1 }",
