...
10. Offer the optional per-user background service only after explaining its effect and receiving confirmation. On Windows, use the selected runtime's built-in `service install` command; do not invent a second Task Scheduler task, PowerShell launcher, PM2 process, or other supervisor. The native Windows task already starts headless at sign-in and keeps StillOn alive with an infinite backoff watchdog. If I confirmed that agent egress needs an already-running local proxy, use only an explicit dedicated service environment file and `stillon service install --env-file <absolute-path>`; do not treat this as permission to configure inbound access or to manage the proxy itself.
